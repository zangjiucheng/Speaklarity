...
import {TiMediaPause, TiMediaPlay, TiStar} from 'react-icons/ti';
import {MdNavigateBefore, MdNavigateNext} from 'react-icons/md';
import {PiMouseLeftClickFill, PiMouseRightClickFill} from 'react-icons/pi';
import {downloadNativeReference} from '../utils/api.tools.ts';
                        ...
                        <div className="flex flex-col h-full w-full">
                            ...
                            <div className="px-8 pb-1">
                                <p className="flex justify-between items-center">
                                    <span>
                                        <span
                                            className="font-semibold">Current Time:</span> <span
                                        className={'font-mono'}>{currentTimeString}</span>
                                        {sentenceScore !== null && <>
                                    ...
                                    <span>
                                        ...
                                        <Button size="sm" isIconOnly variant="flat">
                                            <MdNavigateBefore size={12} className={'scale-[175%]'}/>
                                        </Button>
                                        <Button size="sm" isIconOnly variant="flat" className="mx-1" onPress={() => {
                                            if (wavesurfer) {
                                                wavesurfer.playPause();
                                            }
                                        }}>
                                           {wavesurfer?.isPlaying() ?
                                               <TiMediaPause size={12} className={'scale-[175%]'}/> :
                                               <TiMediaPlay size={12} className={'scale-[175%]'}/>}
                                        </Button>
                                        <Button size="sm" isIconOnly variant="flat">
                                            <MdNavigateNext size={12} className={'scale-[175%]'}/>
                                        </Button>
                                    </span>
                                </p>
                            ...
                            <div className="flex-1 overflow-auto px-8 pb-8 pt-4">
                                <div className="w-full h-full border rounded-xl p-6"
                                    ...
                                    {
                                        props.recAnalysis && props.recAnalysis.sentences.map((sentence, sentence_id) => {
                                            const isCurrent = sentence.audio_timeline.start <= currentTimeSecond && sentence.audio_timeline.end >= currentTimeSecond;
                                            return <Tooltip color="foreground" showArrow content={<div>
                                                <p className="flex items-center"><PiMouseLeftClickFill className="mr-1"
                                                                                                       size={16}/>Seek
                                                    in soundtrack
                                                    above</p>
                                                <p className="flex items-center"><PiMouseRightClickFill className="mr-1"
                                                                                                        size={16}/>Play
                                                    in standard
                                                    English</p>
                                            </div>}>
                                                <span
                                                    ...
                                                    onMouseDown={(evt) => {
                                                        ...
                                                        if (evt.button === 2) {
                                                            console.log('Fetching standard English for sentence:', sentence_id);
                                                            if (props.recAnalysis?.conversation_id)
                                                                downloadNativeReference(props.recAnalysis?.conversation_id, sentence_id).then(result => {
                                                                    if (result.success && result.data) {
                                                                        const audioUrl = URL.createObjectURL(result.data);
                                                                        const audio = new Audio(audioUrl);
                                                                        audio.play();
                                                                    } else {
                                                                        console.error('Failed to fetch native reference:', result.reason);
                                                                    }
                                                                });
                                                        }
                                                    }}
